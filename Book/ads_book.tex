...
\\~\\
Com o latch podemos usar o controle de tempo (chamado de \textbf{clock}) para controlarmos o comportamento oscilante dos circuitos sequenciais.
\\~\\
Os circuitos sequenciais são precisamente os circutos com latch e sinais de controle por clock. São muito releventes para os computadores pois possuem a capaciade de memorização.
\\~\\
Existem dois tipos de circuitos sequenciais muito importantes para a estrutura dos computadores modernos. O primeiro deles é o \textbf{registradores} que são usados para memorizar temporariamente informações em memória maiores que 1 bit. Os \textbf{contadores} usam o clock para a contagem da passagem do tempo.
