...
\\~\\
Podemos criar um contador diretamente no código através da atribuição do valor pela soma. Mas o token de loop \verb|for| recebe um terceiro atributo que é justamente feito para criarmos nossa condição de atualização. Quando colocamos o terceiro parâmetro igual a \verb|contador++|, a cada iteração teremos \verb|+1| atribuído ao contador.
\\~\\
...
\\~\\
\textbf{Comentário: } Cada linguagem podem ter nomes diferentes para esses conceitos, mas o importante é aprender a lógica da coisa.
...
\\~\\
Existem dois tipos de modularização que podemos usar: as que retornar um valor (que chamamos de funções) e as que não retornam valores\footnote{Isso não significa que não podemos printar informação na tela por meio de um procedimento, só significa que o resultado dele não salvará uma variável na memória do computador nem poderá ser atribuída a uma nova variável.} (que chamamos de procedimentos). Abaixo vemos como criar esses dois tipos.

\begin{Verbatim}[tabsize=4,frame=single]
// criando uma funcao

tipo_retorno nome_metodo (par1,par2,..., parn)
{
	comando;
	...
...
void nome_procedimento (par1,par2,...,parn)
{
	comando;
	...
}
...
Temos que lembrar que um procedimento sem c\# sempre vai receber esse token \verb|void| antes de ser criado. Lembremos, também, que um \textbf{método} nada mais é que uma função que será atribuída dentro de um objeto.
\\~\\
Para usarmos um parâmetro não precisamos fazer nenhuma atribuição, basta chamar o nome \verb|nome_procedimento(par1,...,parn)| com os parâmetros preenchidos. No caso de uma função, como ela retorna um resultado, precisamos atribuir o resultado a alguma variável. Isso é feito usando o token de atribuição do seguinte modo: \verb|variavel = nome_metodo(par1,...,parn)|.
\\~\\
