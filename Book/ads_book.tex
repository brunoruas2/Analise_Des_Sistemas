...
\\~\\
Existem dois tipos de modularização que podemos usar: as que retornar um valor (que chamamos de funções) e as que não retornam valores\footnote{Isso não significa que não podemos printar informação na tela por meio de um procedimento, só significa que o resultado dele não salvará uma variável na memória do computador, logo, não poderá ser atribuída a uma nova variável.} (que chamamos de procedimentos). Abaixo vemos como criar esses dois tipos.
...
Temos que lembrar que um procedimento em c\# sempre vai receber esse token \verb|void| antes de ser criado (ainda não aprenderemos o que significa esse termo \verb|static|). Lembremos, também, que um \textbf{método} nada mais é que uma função que será atribuída dentro de um objeto e que temos que definir o tipo da variável de retorno na hora da criação dele.
\\~\\
Para usarmos um parâmetro não precisamos fazer nenhuma atribuição, basta chamar-lo com os parâmetros (par1 até parn) preenchidos. No caso de uma função, como ela retorna um resultado, precisamos atribuir o resultado a alguma variável.
