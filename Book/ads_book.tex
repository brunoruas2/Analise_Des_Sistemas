...
\\~\\
Quanto o método escolhido é o \verb|POST|, os dados são enviados ao servidor no corpo da requisição HTTP e não aparecem na URL. A essa altura você já deve ser capaz de entender as diferenças entre esses dois métodos.
\\~\\
...
\begin{itemize}
	\item \textbf{type} \\
			Identifica o tipo do campo: text, number, password, email, date, submit, radio, checkbox.
	\item \textbf{name} \\
			Nome de identificação do campo.
	\item \textbf{value} \\
			Valor contudo no campo.
	\item \textbf{placeholder} \\
			Valor que aparece quando o campo estiver vazio.
	\item \textbf{required} \\
			Validação automática para evitar o não preenchimento do campo antes da submissão do form.
	\item \textbf{disables} \\
			Inativa o campo e não permite interação mas o user ainda poderá ver.
\end{itemize}
